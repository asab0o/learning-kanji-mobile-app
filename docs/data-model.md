...
`item_key` の形式は **`word:<表記>`**(例 `word:人間`)。2026-09-06 に確定した
(`src/features/quiz/selection.ts` の `toItemKey()`)。

**`words` の行 ID を使わない。** `words` は漢字の樹の葉なので、2字の語は両方の字の樹に
別 ID で入っている(`時間` は 時 と 間 の2行)。行 ID を鍵にすると同じ語が2回出せてしまい、
この表を持つ唯一の目的が果たせない。

`result` 列は入れるが、**読むのは `item_key` だけ**。正答率も連続日数も画面に出さない
