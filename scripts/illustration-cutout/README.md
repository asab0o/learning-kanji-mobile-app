...
**前提その1**: 画像の外周が紙であること。最外周の数pxに暗い額縁が付いていると、
フラッドフィルの種が1つも取れず **1画素も抜けない**(抜きが甘いのではなく素通りする)。
額縁は `frame_depth` が自動で剥がすが、剥がすのは 8px まで。

**前提その2**: 被写体が紙より暗い輪郭線で閉じていること。線が途切れていると、そこから紙の白が
...
`keying.py` の抜き(輪郭線の内側の白が残る・外周の白が抜ける・最外周の額縁を剥がす・淡い色面の粒が埋まる・
...
- **背景が広く残る** … まず `!! 紙が残っているかも` が出ていないか見る(被写体が
  8割方を占めたら出る。紙の矩形が丸ごと残ったときにしか届かない値)。
  よくある原因は最外周の暗い額縁で、8px を超える厚みだと剥がせない —— 生画像側で
  枠を落とす。額縁でなければ紙が白くない生画像なので `WHITE_MARGIN` を上げる。
  部分的に残るケースは自動検知していないので目視で
