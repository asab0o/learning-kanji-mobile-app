...
- **背景が広く残る** … まず `!! 紙が残っているかも` が出ていないか見る(正規化後の
  被写体が 0.65 を超えたら出る。正常な絵の実測は最大 0.502 なので、紙の矩形が丸ごと
  残ったとき — 上限の 0.706 — にしか届かない)。
...
**`--only` を付けずに全走行すると、下の5枚が黙って壊れる。**
`assets/kanji/` の正は生画像 + このツールという建前だが、次の5枚だけ例外になっている。
...
| `enter.png` | 焼き込まれていた枠の縦線(x=918..924)を alpha=0 で消してある(2026-09-08)。生画像に反映されていない | 縦線が戻る |
| `name.png` | 同上。縦線は x=140..158(2026-09-08) | 縦線が戻る |

**全部を焼き直したいときは `--only` で対象を絞るか、この5枚を退避してから走らせる。**
手を動かさずには戻せない(git から戻すことはできる)。
経緯は `docs/log/2026-09.md`。
